...
const mdText = ref('# .33    No.14 ·安全技术· 文章编号:1000—3428(2007)14—0154—03 文献标识码:A 中图分类号:Tp309.1 K-Means 聚类算法在入侵检测中的应用 李  洋 (长沙理工大学计算机与通信工程学院,长沙 410076)\n' +
    '\n' +
    ...
    '的行为是正常行为,与攻击行为相符合的是入侵行为,二者 都不符合的,则认为是异常数据,将其加入到数据仓库中作\n' +
    ...
    ' \n' +
    '的相异度根据对象的属性值计算。聚类分析属观察式学习, 不依赖预先定义的类和训练实例,由此形成的每个簇,可从\n' +
    ...
    '分法首先创建一个初始划分,然后采用一种迭代的重新定位 技术,通过对象在划分间的移动改进划分效果。一个好划分 的一般准则是:同一类中的对象间尽可能" 接近" 或相关, 不同类中的对象间尽可能" 远离" 或不同。当然,还有许多\n' +
    '其它评判划分质量的准则。 \n' +
    ...
    '值。这个过程不断重复,直至准则函数收敛到期望值。由于 实际应用中对象数据选用的度量单位将直接影响聚类分析结 果,不同度量单位可能产生迥异的聚类结构,因此为避免对 度量单位选择的依赖,实际中应先对数据进行标准化处理。\n' +
    '标准化的步骤如下: \n' +
    '给定一个变量 f 的度量值, \n' +
    ...
    '输入 簇的数目 K 和 N 个对象的数据库 输出 K 个簇,满足均方误差函数值最小 \n' +
    '\n' +
    '## 步骤:\n' +
    '\n' +
    ...
    '程度上依赖于收集信息的可靠性和正确性,选择哪些数据表 现用户行为是首要问题。黑客们经常在系统和网络日志文件 中留下踪迹,充分利用这些信息是检测入侵的必要条件。所 选择的特征数据应能充分反映用户行为特征全貌,数据量应 尽量小,提取难度不可太大,还要考虑学习过程的时间、用\n' +
    ...
    'Count\n' +
    'Serror\n' +
    'Same_srv Diff_srv Srv_count Srv_serror Srv_diff_host\n' +
    'Class\n' +
    '序号\n' +
    '1 \n' +
    '1 \n' +
    ...
    '数据集,实验表明,迭代次数总是一个小于数据集的整数, 并与之保持近似线性关系,这无疑对满足适时应用需要有实\n' +
    ...
    '如果不满足上述条件 \n' +
    ' \n' +
    '对其中的异常行为记录作进一步分析,可再次运用\n' +
    'K-means 算法进行二次识别,程序只经过 4 次迭代,划归出 2 类,分别是记录 4、6、12、13、20 和记录 5、19,均方误\n' +
    ...
    '确度上可能略有不足,但其应用便捷,对训练数据集的要求 低,特别是它可以优化或完全抛弃既有模型,对用户行为重 新划分,从中不断挖掘新的潜在模式,使该方法在入侵检测\n' +
    '领域有广泛的应用前景。 \n' +
    '该算法适用于对入侵检测库构建时的初选,为降低误判\n' +
    '率和漏判率,还可对初选后的结果再次分类或配合其它算法\n' +
    '进行合法性分析,提高辨析精度。但是 K-means 方法不适合\n' +
    '发现非凸面形状的簇,或大小差别很大的簇,而且它对" 噪 声" 和孤立点数据较敏感,少量该类数据可能对平均值产生 较大影响,因此,如何" 降噪" 和" 脱敏" 是下一步工作需\n' +
    '要继续研究的问题。 \n' +
    '\n' +
    '## 参考文献\n' +
    '\n' +
...
const summary_lst=[
  ...
  {
    "src": "——入侵检测技术。入侵检测用于识别非授权使用计算机系\n图 1 中引擎观察原始数据并计算用于模型评估的特征;\n统的个体(如黑客)和虽有合法授权但滥用其权限的用户(如内\n检测器获取引擎的数据并利用检测模型评估它是否是一个攻\n部攻击)。现有的入侵检测系统大都采用专家系统或基于统计\n击;数据仓库被用作数据和模型的中心存储地;自适应检测\n的方法,这需要较多的经验,而数据挖掘(data mining)方法的\n模型实时产生,送至检测器实时检测入侵行为。在整个检测\n优势在于它能从大量数据中提取人们感兴趣的、事先未知的\n系统中,自适应检测模型的产生无疑对入侵行为的辨识起着\n知识和规律,而不依赖经验[1]。本文运用数据挖掘中的聚类\n决定作用。如何快速准确地产生检测模型库显得至关重要。\n分析方法,建立入侵检测模型数据库。它的优点是能高度自",
    "summary": "入侵检测技术用于识别非授权使用计算机系统的个体或滥用权利的授权用户。大多数现有的入侵检测系统使用专家系统或基于统计的方法，但这需要大量经验。数据挖掘的优势在于可以从大量数据中提取未知的知识和规律，而不依赖于经验。本文采用了数据挖掘的聚类分析方法，建立了一个入侵检测模型数据库。快速准确地产生检测模型库对于识别入侵行为至关重要。",
    "metadata": {
      "Header 1": "Application Of K-Means Clustering Algorithm In Intrusion Detection",
      "Header 2": "图 **1**  入侵检测框架"
    }
  },
  {
    "src": "动化地分析原有数据,作出归纳性推理,从中挖掘出潜在的\n入侵检测模型能否高效、准确地辨析海量的用户行为数\n模式,预测出客户的行为,更重要的是它能够优化或完全抛\n据,并尽可能降低误判率、漏判率是判断一个入侵检测系统\n弃既有的模型,对入侵行为重新划分并用显示或隐式的方法\n成功与否的标志。数据挖掘技术是一种决策支持过程,它主\n进行描述。仿真实验表明该方法具有较强的实用性和自适应\n要基于人工智能(AI)、机器学习统计等技术,能从大量数据\n功能。本文的聚类分析方法是基于距离的K-平均值(K-means)\n中提取或挖掘知识。其中的聚类分析方法具有可伸缩性、高\n方法,利用此技术实现网络安全目前在国内外都是一种新的\n维性、能处理不同类型属性、可按各种约束聚类等优点,尤\n尝试。\n其适用大型数据库的模式分类[4]。",
    "summary": "这段文本主要讨论了入侵检测模型的重要性，以及它如何通过采用数据挖掘技术来优化并准确分析海量的用户行为数据。这个模型可以预测客户行为，并优化或完全获取已有的模型，以此来降低误判率和漏判率。模型使用的是基于人工智能和机器学习的决策支持过程。文中也提到了一个基于距离的K-平均值方法进行聚类分析，这种方法有诸多优点，尤其擅长处理大型数据库的模式分类。使用这种技术实现网络安全是一个新的尝试，并被证实具有实用性和自适应功能。",
    "metadata": {
      "Header 1": "Application Of K-Means Clustering Algorithm In Intrusion Detection",
  ...
  {
    "src": "入侵检测是对入侵行为的发觉。入侵检测系统将收集到\n聚类按照\" 最大化类内相似性,最小化类间相似性\" 的\n的信息加以分析,判断网络中是否有违反安全策略的行为和\n原则,将数据对象分组为多个类或簇(cluster),同一个簇中的\n遭到攻击的迹象,若找到入侵痕迹,认为与正常行为相符合\n对象具有较高相似度,而不同簇间的对象差别较大,对象间\n的行为是正常行为,与攻击行为相符合的是入侵行为,二者 都不符合的,则认为是异常数据,将其加入到数据仓库中作\n作者简介:李  洋(1974-),男,硕士研究生,主研方向:数据挖掘,\n进一步分析。  \n智能控制,Petri 网理论及应用\n入侵检测系统的基本框架如图 1 所示[2]。  \n收稿日期:2006-08-27    **E-mail**:liyangpeace@sina.com  \n的相异度根据对象的属性值计算。聚类分析属观察式学习, 不依赖预先定义的类和训练实例,由此形成的每个簇,可从\n中 导 出 相 应 规 则 。 聚 类 分 析 算 法 包 括 划 分 法 (patitioning method)、层次法(hierarchical method)、密度法(density method)\n等,其中划分法最常用。  \n2.2  聚类划分法\n给定一个有 N 个对象或元组的数据库,用聚类划分法构\n建数据的 K 个划分,每个划分表示一个聚簇,并且 K≤N,\n即将数据划分为 K 个组时,必须满足如下要求:(1)每个组至\n少包含一个对象;(2)每个对象必须属于且只属于一个组。划\n分法首先创建一个初始划分,然后采用一种迭代的重新定位 技术,通过对象在划分间的移动改进划分效果。一个好划分 的一般准则是:同一类中的对象间尽可能\" 接近\" 或相关, 不同类中的对象间尽可能\" 远离\" 或不同。当然,还有许多\n其它评判划分质量的准则。\n为了达到全局最优,基于划分的聚类要求穷举所有可能\n的划分。在聚类划分中,基于距离的分类采用度量方式,例\n如 K-means、K-medoids 等。当前比较流行的启发式方法首推\nK-means 算法,本文采用此算法对已知用户行为数据库进行\n聚类划分,检测入侵行为。  \n2.3  K-means 算法\nK-means 算法以 K 为参数,把 N 个对象分为 K 个簇,以\n使簇内具有较高的相似度,而簇间的相似度较低。相似度的\n计算根据一个簇中的平均值(视为簇重心)进行。K-means 算法\n的处理过程为:(1)随机选择 K 个对象,每个对象初始代表一\n个簇的平均值或中心。对剩余的每个对象,根据其与各个簇\n中心的距离,将它赋给最近的簇。(2)重新计算每个簇的平均\n值。这个过程不断重复,直至准则函数收敛到期望值。由于 实际应用中对象数据选用的度量单位将直接影响聚类分析结 果,不同度量单位可能产生迥异的聚类结构,因此为避免对 度量单位选择的依赖,实际中应先对数据进行标准化处理。\n标准化的步骤如下:\n给定一个变量 f 的度量值,\n(1)计算平均绝对误差(mean absolute deviation)sf  \n(\n)\n1\n2\n1\n...\np\nf\nf\nf\nf\nf\nf\nf\ns\ny\nm\ny\nm\ny\nm\n= p\n−\n+\n−\n+\n+\n−  \n其中,\n是变量f的p个度量值;m  \n1,...,\np\nfy\nyf\nf是f的平均值,即  \n1\n2\n1 (\n...\np\nf\nf\nf\nm\ny\ny\ny\n= p\n+\n+\n+\nf )\n。  \n(2)计算标准化的度量值(X-Score)  \nm\ny\nx\ni  \ni\n−\n=\n,i=1,2,…,p  \nf\nf\nf\ns  \nf  \n用于判断的准则函数通常采用均方误差和,其定义如  \n式(1)所示:  \n∑\n∑\n=\n∈\n−\n=\nk  \n2                   (1)  \ni\nC\np\ni\ni\nm\np\nE\n1  \n2\n2\n2\n( , )\n...\np\np\ni\nj\ni\nj\ni\nj\nd i j\nx\nx\nx\nx\nx\nx\n=\n−\n+\n−\n+\n+\n−\n(2)  \n| 1   | 2   | 2   | 2   |\n|-----|-----|-----|-----|  \n其中,E是数据库中所有对象的均方误差总和;p表示给\n定的数据对象;mi是簇ci中数据对象的加权平均值(p和mi都是\n多维的);簇ci的数目取决待划分类数。每个对象与簇中心的\n距 离 采 用 欧 几 里 德 距 离 , 其 定 义 如 式 (2) 所 示 , 其 中  \np\ni\ni\ni\ni\nx\nx\nx\n=\n和\n1\n2\n(\n,\n,...,\np )\nj\nj\nj\nj\nx\nx\nx\n=\n是 2 个P维的数据对象。该  \n1\n2\n(\n,\n,...,\n)  \n算法试图找出使均方误差函数值最小的K个划分,令生成的  \n结果尽量紧凑、独立。下面给出了K-means算法的流程,从中  \n可以得到,算法的复杂度为\n,远小于\n,其中,n  \nO(nkt)\n)\n(\nn2\nO  \n是所有对象的数目;k是簇的数目;t是迭代次数(一般k和 t\n均小于n)。鉴于待划分的数据库通常比较大,这种性能还是\n比较优良的。  \nK-means 算法流程如下。\n算法 K-means 基于簇中对象平均值\n输入 簇的数目 K 和 N 个对象的数据库 输出 K 个簇,满足均方误差函数值最小",
    "summary": "这段文本主要讲述了入侵检测系统和数据聚类以及K-means算法。首先，入侵检测系统通过收集和分析信息来检测是否有违反安全策略的行为或攻击迹象。数据聚类则是在原则\"最大化类内相似性，最小化类间相似性\"下，将数据分为多个类或簇。接着文中介绍了聚类划分法，其中包括划分法，层次法和密度法等算法。\n\n特别强调了K-means算法，这个算法的目标是将N个对象分为K个簇，以实现簇内具有较高的相似度，而簇间的相似度较低。K-means算法通过初始化簇的中心，将对象分配至最近的簇，再重新计算每个簇的中心，直到满足某个准则函数。在处理过程中，数据需要进行标准化处理以消除度量单位的影响。最后，文本解释了K-means算法的流程，强调其效率高，适合大规模数据的划分。",
    "metadata": {
      "Header 1": "Application Of K-Means Clustering Algorithm In Intrusion Detection",
      "Header 2": "1  入侵检测系统 2.1  聚类分析"
    }
  },
  {
    "src": "(1)任意选择 K 个对象作为初始簇中心(也是初始平均值);  \n(2)对 K 个对象数据度量值进行标准化处理;\n(3)Repeat;\n(4)计算分配后每个簇中对象的平均值(第一次按初始平均值);\n(5)根据簇中对象平均值,重新将每个对象赋给最类似的簇;\n(6)Until 对象的重新分配不再变化。\n3  实例仿真\n限于篇幅,本文利用上述K-means算法只对一个小型用\n户行为数据库进行实例分类[1],表 1(除Class栏)列出了 20 条\n网络级连接记录的特征数据[5]。须指出的是,入侵检测很大\n程度上依赖于收集信息的可靠性和正确性,选择哪些数据表 现用户行为是首要问题。黑客们经常在系统和网络日志文件 中留下踪迹,充分利用这些信息是检测入侵的必要条件。所 选择的特征数据应能充分反映用户行为特征全貌,数据量应 尽量小,提取难度不可太大,还要考虑学习过程的时间、用\n户行为的时效性等。此例中,特征参数的含义如表 2 所示[6]。",
    "summary": "本文段落描述了K-means聚类算法的流程：首先选择K个对象作为初始的簇中心，然后对这些对象的数据进行标准化处理。然后重复计算每个簇中对象的平均值，并根据平均值重新将对象分配到最相似的簇中，直到对象的分配不再发生变化。另外，本文也展示了一个实例，利用K-means算法对网络级连接记录的特征数据分类。在进行分类时，关键在于选择能充分反映用户行为全貌的特征数据，并考虑学习过程的时间以及用户行为的时效性。",
    "metadata": {
      "Header 1": "Application Of K-Means Clustering Algorithm In Intrusion Detection",
      "Header 2": "步骤:"
    }
  ...
  {
    "src": "参数\nCount\nSerror\nSame_srv Diff_srv Srv_count Srv_serror Srv_diff_host\nClass\n序号\n1\n1\n0.1\n1.0\n0.0\n5.0\n0.0\n0.6\n正常\n2\n1\n0.0\n1.0\n0.0\n4.0\n0.0\n0.5\n正常\n3\n15\n0.88\n0.88\n0.12\n25.0\n0.5\n0.0\n攻击\n4\n6\n0.75\n0.3\n1.0\n6.0\n0.0\n0.0\n异常\n5\n8\n0.95\n0.25\n0.75\n7.0\n0.6\n0.3\n异常\n6\n7\n0.82\n0.18\n0.9\n6.0\n0.0\n0.0\n异常\n7\n1\n0.0\n0.55\n0.67\n2.0\n0.0\n0.0\n正常\n8\n1\n0.0\n1.0\n0.0\n2.0\n0.0\n0.0\n正常\n9\n1\n0.2\n1.0\n0.0\n3.0\n0.0\n0.33\n正常\n10\n2\n0.1\n0.9\n0.1\n5.0\n0.0\n0.55\n正常\n11\n1\n0.0\n0.75\n0.0\n6.0\n0.0\n0.5\n正常\n12\n6\n0.8\n0.1\n0.9\n6.0\n0.2\n0.2\n异常\n13\n7\n0.85\n0.05\n0.85\n6.0\n0.1\n0.0\n异常\n14\n1\n0.0\n0.85\n0.0\n5.0\n0.0\n0.45\n正常\n15\n1\n0.2\n0.65\n0.3\n4.0\n0.0\n0.65\n正常\n16\n1\n0.2\n0.5\n0.5\n4.0\n0.0\n0.0\n正常\n17\n2\n0.0\n0.6\n0.4\n5.0\n0.0\n0.1\n正常\n18\n1\n0.0\n0.67\n0.33\n6.0\n0.0\n0.2\n正常\n19\n7\n0.9\n0.33\n0.67\n7.0\n0.6\n0.2\n异常\n20\n6\n0.8\n0.0\n1.0\n6.0\n0.0\n0.0\n异常",
    "summary": "该文本是一份基于各种参数（包括'Count'，'Serror'，'Same_srv'，'Diff_srv'等）的分类列表，其中记录了20条序号的数据，并且针对每组数据进行了类别划分，类别分为'正常'，'攻击' 和 '异常'。这些数据或许被用来对网络活动进行评估和分析。",
    "metadata": {
      "Header 1": "Application Of K-Means Clustering Algorithm In Intrusion Detection",
      "Header 2": "表 **1**  网络连接记录及分类"
    }
  },
  {
    "src": "参数\n含义\n类型  \n在一个时间窗口内目标主机是与当前连接  \nCount\n相同的连接次数(以下属性针对相同主机的\nUint\n连接)\nSerror\n出现 SYN 错误的连接百分比\nUint\nSame_srv\n目标端口(service)相同的连接所占的百分比\nUint\nDiff_srv\n目标端口(service)不同的连接所占的百分比\nUint\n目标端口(service)与当前连接相同的连接次\nUint\nSrv_count\n数(以下属性针对相同服务的连接)\nSrv_serror\n出现 SYN 错误的连接百分比\nUint\nrv_diff_host\n目标主机不同的连接所占的百分比\nUint\nClass\n分类结果\nChar  \n此算法用 C++编程实现,实验数据均取自 PIII566 微机。\n按上述流程及所选择的特征数据,程序经过 8 次迭代即识别\n出攻击、异常和安全 3 种类型的记录(也可根据需要设定其它\n分类数),均方误差和为 57.004 327。将该算法用于不同大小\n数据集,实验表明,迭代次数总是一个小于数据集的整数, 并与之保持近似线性关系,这无疑对满足适时应用需要有实\n际作用。此例中,聚类分析后识别出的结果如表 2 的 Class\n栏所示。从中可以看出,运行 K-means 聚类后,记录 3 是唯\n一具有攻击倾向的记录;而记录 4~6、12、13、19、20 是具\n有异常行为模式的 7 条记录,需要进一步观察;剩下的记录\n1、2、7~11、14~18 则是安全的。K-means 算法按特征参数的\n性质,把该网络级行为数据集归为 3 类。\n图 2 是运行算法前后,聚类结果的对照。从图 2(b)中可\n以看出,具有异常或攻击行为模式的记录,同正常行为记录\n有效隔离开来。对 K-means 聚类结果进行分析后,总结出如\n表 3 所示的 3 类规则及其含义,这些规则中的正常与攻击行\n为模式可作为入侵检测模式保留在数据仓库中,用以预测和\n判断用户行为合法性的依据。  \n22  \n20\n20\n18\n18\n16\n16\n14\n14\n12\n12\n10\n10\n8 8 6 6 4 4  \n(a)\n22\n22\n20\n20\n18\n18\n16\n16\n14\n14\n12\n12\n10\n10\n8\n8\n6\n6\n4\n4",
    "summary": "这段文本介绍了一种利用C++编程实现的网络行为数据分析算法。算法通过多个参数（如连接次数、SYN错误比例、目标主机或端口的变化等）对行为进行分析，划分为\"攻击\"、\"异常\"、\"安全\"三种状态。该算法模型在8次迭代后成功确认出这三种记录类型，均方误差和为57.004327。实验结果显示迭代次数总是一个小于数据集的整数，并与之保持近似线性关系，证明了算法的有效性。对分析的数据进行K-means聚类后，可以进一步有效地隔离出异常或攻击行为模式与正常行为的记录，这对预测和判断用户行为的合法性是非常有用的。",
    "metadata": {
      "Header 1": "Application Of K-Means Clustering Algorithm In Intrusion Detection",
      "Header 2": "表 **2**  特征参数"
    }
  ...
  {
    "src": "规则\n含义\n在一个时间窗口内,目标主机与当前连接相同的次数大于等于\n攻击\n15;同一主机的连接中出现 SYN 错误的百分比大于等于 88%,\n且目标端口与当前连接相同次数大于等于 25\n在一个时间窗口内,目标主机与当前连接相同次数大于等于 6;\n异常\n同一主机连接中出现 SYN 错误的百分比大于 75%,且目标端口\n与当前连接相同次数大于等于 6\n正常\n如果不满足上述条件  \n对其中的异常行为记录作进一步分析,可再次运用\nK-means 算法进行二次识别,程序只经过 4 次迭代,划归出 2 类,分别是记录 4、6、12、13、20 和记录 5、19,均方误\n差和为 18.055 98。图 3 是再次应用聚类算法识别异常行为记\n录后,分类出的结果。对分类的记录数据进行合理性分析,\n可以得出记录 4、6、12、13、20 的用户行为不具备攻击特性,\n可提高其安全等级;而记录 5、19 相同服务的连接中出现 SYN\n错误达 60%,同一主机连接中出现 SYN 错误超过 90%,应\n予以重点监控。\n22\n22\n20\n20  \n18\n18\n16\n16\n14\n14\n12\n12\n10\n10\n8\n8\n6\n6\n4\n4\n2\n2 0\n0. 2\n0. 4\n0. 5\n0. 6\n0  \n图 **3**  二次分类结果",
    "summary": "这段文本描述了一个通过统计并分析网络连接异常情况来洞察并预防网络攻击的方法。规则设定为在特定时间窗口内，如果同一目标主机的连接次数达到15次，和SYN错误比例超过88%，且相同目标端口连接超过25次，则被定义为攻击。如果连接次数达到6次，SYN错误率超过75%，和同样的目标端口连接超过6次，则被视为异常。如果不满足上述条件则视为正常。异常行为会被记录并进一步分析。通过应用K-means算法进行二次识别，结果被划分为两类，其中记录4、6、12、13、20的用户行为没有攻击特性，可提高其安全等级;记录5、19显示出同一服务的连接中出现了60%的SYN错误，同一主机的连接中出现超过90%的SYN错误，因此需要重点监控。",
    "metadata": {
      "Header 1": "Application Of K-Means Clustering Algorithm In Intrusion Detection",
      "Header 2": "图 **2**  聚类结果的对照 表 **3**  聚类检测规则"
    }
  },
  {
    "src": "本文运用聚类分析中的 K-means 算法分析用户行为数据\n库,从中筛选出不安全的用户,也可以凭借该算法分列安全\n等级,构建入侵检测库。\n与传统方法相比,基于聚类的 K-means 算法在检测的精\n确度上可能略有不足,但其应用便捷,对训练数据集的要求 低,特别是它可以优化或完全抛弃既有模型,对用户行为重 新划分,从中不断挖掘新的潜在模式,使该方法在入侵检测\n领域有广泛的应用前景。\n该算法适用于对入侵检测库构建时的初选,为降低误判\n率和漏判率,还可对初选后的结果再次分类或配合其它算法\n进行合法性分析,提高辨析精度。但是 K-means 方法不适合\n发现非凸面形状的簇,或大小差别很大的簇,而且它对\" 噪 声\" 和孤立点数据较敏感,少量该类数据可能对平均值产生 较大影响,因此,如何\" 降噪\" 和\" 脱敏\" 是下一步工作需\n要继续研究的问题。",
    "summary": "本文运用K-means算法分析用户行为数据库以筛选出不安全用户和分列安全等级构建入侵检测库。尽管此方法可能在精确度上略有不足，但它操作便利，对训练数据集要求低，并且能够发掘新的潜在模式。同时，使用此方法初选入侵检测库可以降低误判率和漏判率，结合其他算法能提高辨析精度。缺点是该方法不适用于发现非凸形状或大小差别大的簇，对\"噪声\"和孤立点数据敏感。后续研究需要继续探索如何降噪和脱敏。",
    "metadata": {
      "Header 1": "Application Of K-Means Clustering Algorithm In Intrusion Detection",
      "Header 2": "4 结语"
    }
  ...
  {
    "src": "1 Lee W, Stolfo S J, Mok K W. Mining Audit Data to Build Intrusion\nDetection Models[EB/OL]. 2001. http://www.yahoo.com.\n2 胡建斌. 入侵检测技术[DB/OL]. 北京大学网络与信息安全研究\n室, 2004.\n3 Han J, Kamber M. 数据挖掘概念与技术[M]. 范  明, 孟小峰, 译.\n北京: 机械工业出版社, 2001.\n4 宁玉杰, 郭晓淳. 基于数据挖掘技术的网络入侵检测系统[J]. 计\n算机测量与控制, 2002, 10(3).\n5 戴英侠, 连一峰, 王  航, 等. 系统安全与入侵检测[M]. 北京: 清\n华大学出版社, 2002.\n6 罗守山. 入侵检测[M]. 北京: 邮电大学出版社, 2004.",
    "summary": "这些文献资源主要关注了网络安全和入侵检测等主题。除了一篇英文文献讨论了如何运用审计数据来建立入侵检测模型外，其他的都是关于入侵检测技术以及建立基于数据挖掘的网络入侵检测系统的中文著作和文章。这些著作出自中国的各大高校和研究机构，反映了这一领域在中国的研究情况。",
    "metadata": {
      "Header 1": "Application Of K-Means Clustering Algorithm In Intrusion Detection",
      "Header 2": "参考文献"
    }
